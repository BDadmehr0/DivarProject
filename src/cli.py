...
from database import init_db, get_setting, set_setting, get_user_by_username, change_password, create_admin_user
from auth import hash_password, verify_password
from config import DEFAULT_MONITOR_INTERVAL, DEFAULT_DAILY_PAGES
...
def change_password_cmd(username: str = typer.Option("admin", help="نام کاربری")):
    ...
    async def _run():
        ...
        from database import change_password as db_change_password
...
def settings():
    ...
    async def run():
        ...
        table.add_row("monitor_interval", await get_setting("monitor_interval", str(DEFAULT_MONITOR_INTERVAL)))
        table.add_row("daily_pages", await get_setting("daily_pages", str(DEFAULT_DAILY_PAGES)))
        console.print(table)
    asyncio.run(run())
